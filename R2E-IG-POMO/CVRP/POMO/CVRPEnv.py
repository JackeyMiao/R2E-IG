...
class CVRPEnv:
    ...
    def load_problems(self, batch_size, aug_factor=1, problems=None, distribution=None,load_path = None, copy=None, episode=None, cumulate_weight=None):
        ...
        if problems is not None:
            ...
            if copy is None:
                if not self.FLAG__use_saved_problems:
                    depot_xy, node_xy, node_demand = get_random_problems(batch_size, self.problem_size, self.distribution, load_path=load_path, episode=episode, cumulate_weight=cumulate_weight)
                    # depot_xy, node_xy, node_demand = get_random_problems(batch_size, self.problem_size,self.distribution,load_path = load_path)
                else:
                ...
                depot_xy, node_xy, node_demand = copy
    ...
    def _get_travel_distance(self):
        if self.env_params['load_raw'] is not None:
            filename= self.env_params['load_raw']
            import pickle
