...
class CVRPModel(nn.Module):
    ...
    def pre_forward(self, reset_state, attn_type=None):
        ...
        if attn_type is not None:  # layer loss
            if attn_type == 'no':  # no attn loss
                embedding, self.encoded_nodes, moe_loss = self.encoder(depot_xy, node_xy_demand, attn_type)
            else:  # attn loss
                embedding, self.encoded_nodes, attn, moe_loss = self.encoder(depot_xy, node_xy_demand, attn_type)
            hidden = self.encoded_nodes
        else:
            self.encoded_nodes, moe_loss = self.encoder(depot_xy, node_xy_demand)
    ...
    def forward(self, state, route=None, return_probs=False, teacher=False):
        ...
        if state.selected_count == 0:  # First Move, depot
            ...
            probs, moe_loss = self.decoder(encoded_last_node, state.load, ninf_mask=state.ninf_mask, last_node_idx=state.current_node)
            self.aux_loss += moe_loss
...
class CVRP_Encoder(nn.Module):
    ...
    def forward(self, depot_xy, node_xy_demand, attn_type =None):
...
class CVRP_Decoder(nn.Module):
    ...
    def set_kv(self, encoded_nodes):
        ...
        self.graph_embedding = multi_head_attention(q, k, v).mean(dim = 1, keepdim = True).repeat((1, encoded_nodes.size(1) - 1, 1))
    ...
    def forward(self, encoded_last_node, load, ninf_mask, last_node_idx=None):
...
class ResExpert_wo_silu(nn.Module):
    def __init__(self, embedding_dim, intermediate_dim):
        super(ResExpert_wo_silu, self).__init__()
        self.fc1 = nn.Linear(embedding_dim, intermediate_dim)
        self.act = nn.ReLU()
        self.fc2 = nn.Linear(intermediate_dim, embedding_dim)
        self.shortcut = nn.Linear(embedding_dim, embedding_dim)
...
class ResExpert_wo_shortcut_linear(nn.Module):
    def __init__(self, embedding_dim, intermediate_dim):
        super(ResExpert_wo_shortcut_linear, self).__init__()
        self.fc1 = nn.Linear(embedding_dim, intermediate_dim)
        self.act = nn.SiLU()
        self.fc2 = nn.Linear(intermediate_dim, embedding_dim)
    def forward(self, x):
...
class ResExpert_wo_res(nn.Module):
    def __init__(self, embedding_dim, intermediate_dim):
        super(ResExpert_wo_res, self).__init__()
        self.fc1 = nn.Linear(embedding_dim, intermediate_dim)
        self.act = nn.SiLU()
        self.fc2 = nn.Linear(intermediate_dim, embedding_dim)
    def forward(self, x):
...
class MOELayer(nn.Module):
    def __init__(
        ...
        self.MODULE_REGISTRY = {
            "origin": ExpertNetwork,
            "Res": ResExpert,
            "Res_wo_shortcut": ResExpert_wo_shortcut_linear,
            "Res_wo_silu": ResExpert_wo_silu,
            "Res_wo_res": ResExpert_wo_res
        }
