...
def get_random_problems(batch_size, problem_size, distribution, load_path=None,episode=None, cumulate_weight=None):
    ...
    if load_path is not None:
        ...
        if distribution['data_type'] == 'uniform':
            ...
            std = distribution['std']
            for j in range(batch_size):
                if j < batch_size * cumulate_weight[0]:
                    class_type = np.array(['uniform'])
                elif j < batch_size * cumulate_weight[1]:
                    class_type = np.array(['cluster'])
                else:
                    class_type = np.array(['mixed'])
